...
if ship_schedule.present?
  json.ship_schedule_detail "#{ship_schedule.ship.try(:name)} #{ship_schedule.estimated_departure_sour_at} - #{ship_schedule.estimated_arrived_dest_at}"
  json.ship_schedule_id @record.ship_schedule_id
  json.ship_schedule_path port_path(@record.ship_schedule_id)
end
...
if agent.present?
  ...
  json.agent_path port_path(@record.agent_id)
end
